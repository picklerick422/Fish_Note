...
import { ArrowRight, ExternalLink, MoreHorizontal, Pin, PinOff, Trash2 } from 'lucide-react'
          ...
          {recent.map((note, i) => (
            <ContextMenu key={note.id}>
                ...
                >
                  ...
                  <div className="flex items-center gap-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-36 rounded-r-md" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenuItem className="gap-2 text-[13px]" onClick={() => { navigate(`/notes?open=${note.id}`) }}>
                          <ExternalLink size={14} />
                          打开
                        </DropdownMenuItem>
                        <DropdownMenuItem className="gap-2 text-[13px]" onClick={() => togglePin(note.id)}>
                        ...
                        <DropdownMenuItem
                          ...
                          onClick={() => { deleteNote(note.id); notify.success('已移入回收站') }}
                        >
                          <Trash2 size={14} />
                          删除
                        </DropdownMenuItem>
              ...
              <ContextMenuContent className="w-36 rounded-r-md" onClick={(e) => e.stopPropagation()}>
                <ContextMenuItem className="gap-2 text-[13px]" onClick={() => { navigate(`/notes?open=${note.id}`) }}>
                  <ExternalLink size={14} />
                  打开
                </ContextMenuItem>
                <ContextMenuItem className="gap-2 text-[13px]" onClick={() => togglePin(note.id)}>
                  {note.pinned ? <PinOff size={14} /> : <Pin size={14} />}
                  {note.pinned ? '取消置顶' : '置顶'}
                </ContextMenuItem>
                <ContextMenuSeparator />
